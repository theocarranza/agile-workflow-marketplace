...
description: >
  Analyze user instructions and safely amend an entire Azure DevOps or AI Codex Ledger
  work-item tree. Use when a user supplies corrections or adjustments for an Epic or Feature
  and expects the Epic, Features, User Stories, and child Tasks to be scanned, placed, reviewed,
  and updated consistently. This skill gathers and presents a change set before any write,
...
- `./references/backup-and-ledger.md` — backup, revision, Obsidian, and recovery rules.
- `./references/approval-and-persistence.md` — change-set approval and delegated writers.
- `../../references/ticket-structure.md` — ledger structure and section order.
...
2. Collect the reference Epic or Feature (Azure id/URL, Ledger path, or filesystem path).
...
Read `backup-and-ledger.md` before resolving the source. Create a backup of the entire tree before
...
proposing changes. Read each node and retain its original revision. When a local Ledger exists,
...
Update related Ledger Implementation Plans when the amendment changes scope, acceptance coverage,
or task sequencing. Read back every changed Azure item and Ledger file, verify parent links and
...
Report backup location, scanned tree counts, approved placements, changed nodes, Task-list and
...
- If Azure and Ledger disagree, stop and present the conflict; do not guess which copy wins.
