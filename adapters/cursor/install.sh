#!/usr/bin/env bash
# Cursor one-liner:
#   curl -fsSL https://raw.githubusercontent.com/OWNER/REPO/REF/adapters/cursor/install.sh \
...
bootstrap_and_reexec() {
  ...
  set +e
  "$tmp/repo/adapters/${HOST}/install.sh" "${original[@]}"
  status=$?
  set -e
  rm -rf "$tmp"
  exit "$status"
...
if ROOT="$(resolve_local_root)"; then
  ...
  source "$ROOT/adapters/_install_lib.sh"
  INSTALL_HOST="$HOST"
  install_main --root "$ROOT" "$@"
else
  bootstrap_and_reexec "$@"
fi
